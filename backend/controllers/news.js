...
const news = async (req, res, next) => {
  ...
  try {
    const listNews = await axios.get(
      `https://api.nytimes.com/svc/search/v2/articlesearch.json?begin_date=20150101&end_date=${dateforSeart}&facet=false&page=${page}&q=${search}&sort=newest&api-key=${API_KEY}`
    );
